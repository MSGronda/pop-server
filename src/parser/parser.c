...
// EXP: funciones deben ser estaticas para poder acceder al array estatico 
...
void parser_init(input_parser * parser){
    ...
    parser->current_command.argument = calloc(MAX_ARG_LENGTH + 1, sizeof(char)); 
...
command_state parser_feed(input_parser * parser, const char c, bool * finished){
    ...
    else if(parser->state == WITH_ARGS_STATE){  
        with_arguments_state(parser, c, finished, current_command);
    }
    else if(parser->state == NO_ARGS_STATE){ 
...
static void command_recognition(input_parser * parser, const char c, bool * finished, command_instance * current_command){
    if(c != '\n'){
        ...
        // EXP: itero por todos los comandos 
        for(int i = 0; i < ALL_CMD_SIZE; i++){
            if(!parser->invalid_type[i]){

                // EXP: comparo el caracter de entrada con el caracter del comando
                if(toupper(c) != all_command_info[i].name[parser->line_size]){
                    ...
                    // EXP: inicializo argumentos
                    if(all_command_info[i].max_args > 0) {

                        // EXP: requiero de un nuevo buffer
                        if(current_command->argument == NULL) {
                            current_command->argument = malloc(MAX_ARG_LENGTH + 1);
                        }

                        // EXP: puede haber un argumento anterior que no se libero => reseteo el buffer de argumento
                    ...
                    // EXP: ya encontre un comando, no hace falta seguir iterando 
                    break;
                }
            }
...
static void with_arguments_state(input_parser * parser, const char c, bool * finished, command_instance * current_command){
    ...
    // Indica que deberia venir argumento
    ...
    // Entro aqui solo si estoy esperando que pase un argumento 
    else if((c != '\r' && c != '\n') || (c == ' ' && current_command->type == CMD_PASS && parser->args_count == 1)){
        if(parser->is_expecting_new_arg){

            // EXP: se la pasan mas argumentos de los que requiere
            if(parser->args_count >= all_command_info[current_command->type].max_args){
                parser->state = COMMAND_ERROR_STATE;
            }

            // EXP: tengo que aumentar ahora porque luego no entra a el if que lo contiene
    ...
    // EXP: entro en la seccion de este comando. pueden existir mas en el buffer, manejanmos uno a la vez
    
    else if(c == '\r'){
        parser->correctly_formed = 1;

        if(all_command_info[parser->current_command.type].max_args > 0){
            current_command->argument[parser->arg_length > 0 ? parser->arg_length-1: 0] = 0;
        }
        
        // EXP: comando sin argumentos 
        if(parser->args_count == 0 && all_command_info[current_command->type].min_args == 0){
            parser->state = NO_ARGS_STATE;
        }

        // EXP: no tiene la cantidad correcta de argumentos 
        ...
        // EXP: null termination del argumento
        if(all_command_info[parser->current_command.type].max_args > 0){
            current_command->argument[parser->arg_length-1] = 0; 
        }

        // EXP: encontro un comando y es valido 
    ...
    // EXP: ni pudo reconocer el argumento
    else {
        parser->correctly_formed = 0;
        parser->state = COMMAND_ERROR_STATE;
        handle_parsed(current_command, parser, finished, true);
...
static void no_arguments_state(input_parser * parser, const char c, bool * finished, command_instance * current_command){
    ...
    // EXP: si no tiene argumentos => deberia termianr en un \r, \n o con espacios (permitimos esto por mas que no sea requerido)
