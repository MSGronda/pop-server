...
void mng_request_to_buffer(mng_request * request, uint8_t * converted, size_t * converted_len) {
    ...
    memcpy(converted + offset, &request->auth_token, sizeof(request->auth_token));
    offset += sizeof(request->auth_token);
    ...
    memcpy(converted + offset, &request->length, sizeof(request->length));
